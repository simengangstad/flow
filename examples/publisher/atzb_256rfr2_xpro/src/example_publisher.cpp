/**
 * @brief This example implements a simple publisher on the ATZB-256RFR2-XPRO
 ...
 * error.
 */
static void reset_callback() { wdt_reset_mcu(); }

/**
 * @brief This callback is used to disable the watch dog before we go to
 * sleep.
 */
 ...
 * transmission.
 */
static void payload_update_callback(
    etl::vector<uint8_t, mesh::MAX_TRANSMISSION_PACKET_SIZE>& data) {
...
auto main() -> int {
    ...
    // We register a callback here so that we are notified before the
    // publisher module goes to sleep. With this, we can disable the watch
    // dog before going to sleep.
